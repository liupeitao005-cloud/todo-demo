...
public interface TodoReviewplanMapper {
    ...
    @Update("UPDATE todo_reviewplan SET is_finish = 1, finish_time = NOW(), update_time = NOW() WHERE id = #{id} AND user_id = #{userId} AND is_finish = 0")
    int finish(@Param("id") Long id, @Param("userId") Long userId);

    @Select("SELECT id,user_id,review_task_id,review_time,is_finish,finish_time,create_time,update_time FROM todo_reviewplan WHERE id = #{id} AND user_id = #{userId}")
    TodoReviewplan selectById(@Param("id") Long id, @Param("userId") Long userId);
