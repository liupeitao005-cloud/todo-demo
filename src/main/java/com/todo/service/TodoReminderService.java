...
public class TodoReminderService {
    ...
    public Result<String> createReminder(TodoReminderDTO dto) {
        ...
        if (!"desktop".equals(channel) && !"app".equals(channel)) {
            return Result.fail("提醒渠道只能是 desktop 或 app");
        }
    ...
    public Result<List<TodoReminder>> pendingDesktopReminder() {
        return pendingReminderByChannel("desktop");
    }

    public Result<List<TodoReminder>> pendingAppReminder() {
        return pendingReminderByChannel("app");
    }

    private Result<List<TodoReminder>> pendingReminderByChannel(String channel) {
        Long userId = UserContext.getUserId();
        if (userId == null) return Result.fail("未登录");
